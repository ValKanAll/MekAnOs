...
def read_dataset_info(_path=path, dataset_name=None):
    ...
    available_scans_list = pd.read_excel(_path, sheet_name='Dataset_list', engine='openpyxl')['Available_scans'].tolist()
        ...
        try:
            ...
            if dataset_name:
                if dataset_name == dataset + ' ({})'.format(year):
                    return Dataset(dataset + ' ({})'.format(year), xl_dataset, main_folder, scans_list)
            else:
                datasets += [Dataset(dataset + ' ({})'.format(year), xl_dataset, main_folder, scans_list)]
