...
class qctma(object):
    ...
    def __init__(self, dcm_path=None, nrrd_path=None, mesh_path="", gl2density=lambda x: x, density2E=lambda x: x,
                 E2density=None,
                 deltaE=0, coef_poisson=0.3,
        ...
        if self.dcm_path and self.nrrd_path:
            raise ValueError("A dicom path and nrrd path were given, choose one")
        elif not self.dcm_path and self.nrrd_path:
            self.image_type = "nrrd"
        elif self.dcm_path and not self.nrrd_path:
            self.image_type = "dcm"
        elif not self.dcm_path and not self.nrrd_path:
            raise ValueError("No dicom path nor nrrd path were given")
        ...
        if self.nb_process > 1:
            warnings.warn(
                "WARNING: More than 1 process will be used. Check if your code runs inside <if __name__ == '__main__':> statement !")
    ...
    def process(self, save_mesh_path=""):
        ...
        if self.image_type == "dcm":
            self.load_dicom()
        elif self.image_type == "nrrd":
            self.load_nrrd()
        """res_im = self.image_mat - self.image_mat2
        max_value = 0
        n1, n2, n3 = res_im.shape
        for i in range(n1):
            for j in range(n2):
                for k in range(n3):
                    max_value = max(abs(res_im[i, j, k]), max_value)

        print("max of diff image", max_value)
        print("x", self.positions_x - self.positions_x2)
        print("y", self.positions_y - self.positions_y2)
        print("z", self.positions_z - self.positions_z2)"""
        self.convert2E()
    ...
    def load_nrrd(self):
        """
        Load the data in the NRRD file and extract the related parameters.
        """
        print("Reading nrrd file...")
        array, header = nrrd.read(self.nrrd_path)
        origin = np.array(header['space origin'])
        space_directions = header['space directions']
        sizes = header['sizes']
        print("NRRD reading done.\n")
        self.image_mat = np.array(array).T  # Pixel values

        # Extracting constant values along each slice of the dicom
        orientation_x, orientation_y, orientation_z = float(space_directions[0][0]), \
                                                      float(space_directions[1][1]), \
                                                      float(space_directions[2][2])
    ...
    def create_materials(self):
        ...
        self.e_elems = np.concatenate(
            save_list)  # Integrals of interp over each elem (i.e. Young's modulus of each elem)
    ...
    def e_pool2density(self):
        if self.E2density:
            self.density_pool = self.E2density(self.e_pool)
        else:
            self.density_pool = self.inv_num(self.density2E, self.e_pool)

    def save_mesh(self, save_mesh_path, report=False):
        """
